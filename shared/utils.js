...
const ICONS = {
  ...
  userPlus:'<circle cx="9" cy="8" r="3.5"/><path d="M2 21c0-3.5 3-5 7-5c1.2 0 2.3.2 3.2.5M17 14v6M20 17h-6"/>',
...
/* ============================================================================
 *  THEME COLOR (EDU-003) — lets Settings → Appearance accept ANY color, not
 *  just the 6 hardcoded themes in shared/tokens.css. Each preset theme sets
 *  5 vars per mode (--brand, --brand-hover, --brand-subtle, --header-bg,
 *  --accent); ThemeColor.derive(hex) computes the same 5 vars for an
 *  arbitrary hex, for both light and dark mode, following the same
 *  relationships already visible across the presets (hover is darker than
 *  brand in light mode / lighter in dark mode; subtle is a near-white tint in
 *  light mode / a near-black tint in dark mode; header is a touch darker than
 *  brand in light mode and much darker in dark mode). Each app applies these
 *  as inline custom properties on <html> (higher specificity than the
 *  [data-theme] rules in tokens.css) when theme==='custom', and clears them
 *  when a preset theme is chosen so the stylesheet rules take back over.
 * ============================================================================ */
const ThemeColor={
  hexToRgb(hex){
    let h=(hex||'').replace('#','');
  ...
  rgbToHex({r,g,b}){ return '#'+[r,g,b].map(x=>Math.max(0,Math.min(255,Math.round(x))).toString(16).padStart(2,'0')).join(''); },
  rgbToHsl({r,g,b}){
    r/=255; g/=255; b/=255;
  ...
  _withL(hsl,l){ return this.rgbToHex(this.hslToRgb({h:hsl.h,s:hsl.s,l})); },
  _shiftL(hex,delta){ const hsl=this.rgbToHsl(this.hexToRgb(hex)); return this._withL(hsl,Math.max(4,Math.min(96,hsl.l+delta))); },
  derive(hex){
    const base=this.normalizeHex(hex);
    const hsl=this.rgbToHsl(this.hexToRgb(base));
    const tintS=Math.min(100,hsl.s*0.6+20);
    return {
      light:{ brand:base, brandHover:this._shiftL(base,-14), brandSubtle:this._withL({h:hsl.h,s:tintS},94), headerBg:this._shiftL(base,-8), accent:base },
      dark:{ brand:this._shiftL(base,10), brandHover:this._shiftL(base,20), brandSubtle:this._withL({h:hsl.h,s:tintS},18), headerBg:this._withL({h:hsl.h,s:tintS},24), accent:this._shiftL(base,10) },
    };
  },
};
/* Broad quick-pick palette shown alongside the native color input + hex field
   (curated hues across the spectrum, not tied to any single theme). */
const THEME_PALETTE=['#EF4444','#F97316','#F59E0B','#EAB308','#84CC16','#22C55E','#10B981','#14B8A6','#06B6D4','#0EA5E9','#3B82F6','#6366F1','#8B5CF6','#A855F7','#D946EF','#EC4899','#F43F5E','#64748B'];
